...
const TRANSITION_RULES = [
  {
    from: ['new'],
    to: 'in_quarantine',
    event: 'feeding.reminder.completed2',
    description: 'Pet moved to quarantine after feeding setup'
  },
  {
    from: ['in_quarantine'],
    to: 'healthy',
    event: 'status.update.requested2',
...
export const config = {
  ...
  subscribes: ['js.pet.created2', 'js.feeding.reminder.completed2', 'js.pet.status.update.requested2'],
  emits: [],
...
export const handler = async (input, context) => {
  ...
  try {
    ...
    let rule;
    if (eventType === 'status.update.requested2' && requestedStatus) {
    ...
    if (!rule) {
      const reason = eventType === 'status.update.requested2' 
      ...
      if (emit) {
        await emit({
          topic: 'js.lifecycle.transition.rejected2',
          data: {
            petId,
            currentStatus: pet.status,
            requestedStatus,
    ...
    if (emit) {
      await emit({
        topic: 'js.lifecycle.transition.completed2',
        data: {
          petId,
          oldStatus,
          newStatus: rule.to,
        ...
        if (emit) {
          await emit({
            ...
            data: {
              ...
              oldStatus,
              newStatus: rule.to,
              eventType: 'status.update.requested2',
              description: progression.description,
              automatic: true,
              timestamp: Date.now()
            }
