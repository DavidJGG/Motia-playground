...
export const config = {
  ...
  description:
    'Sets the next feeding reminder for a pet and updates its status',
  subscribes: ['js.feeding.reminder.enqueued2'],
  emits: ['js.feeding.reminder.completed2'],
  ...
  try {
    ...
    if (streams?.petCreation && traceId) {
      ...
      if (!updatedPet.symptoms || updatedPet.symptoms.length === 0) {
        ...
        await new Promise((resolve) => setTimeout(resolve, 1000))
        await streams.petCreation.set(traceId, 'message', {
          message: `Health check failed for ${
            updatedPet.name
          } - symptoms detected: ${updatedPet.symptoms.join(', ')}`,
        })
    ...
    if (emit) {
      await emit({
        topic: 'js.feeding.reminder.completed2',
        data: {
          petId,
